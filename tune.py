...
def build_tasks(base_commands, remaps, dirs, choices, output, run=None, first=False):
    param_dicts = []
    for choice in choices:
        param_dicts.extend(sweep(choice))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="output", help="root dir for all experiments")
    parser.add_argument("--config", default="params.yaml", help="config file for this group of experiments ")
    parser.add_argument("--first", default=False, action='store_true', help="Only run the first task")
    parser.add_argument("--run", default=None, type=str, help="choose which command to run, default to all")
    args = parser.parse_args()
    base_commands, remaps, dirs, resources, choices = parse_yaml(args.config)
    tasks = build_tasks(base_commands, remaps, dirs, choices, args.output, run=args.run, first=args.first)
